...
| **A full task view** | Core's task drawer, over the lanes on the right — a third of the board wide, never narrower than 22rem: status menu, worker, per-task model override, **editable skills**, description (editable), result and last summary, dependencies, comments, activity, run history — plus a 200px worker log and the context meter. |
...
| **A log that reads like a transcript** | A worker log is a terminal transcript, and the big view draws it as one: the CLI's response frames are redrawn at the width of the panel (title kept, box drawing dropped) and re-fit on every resize, an inline `review diff` gets a rail with `+` in green and `-` in red, and `[kanban-worker-exit] rc=…` — the one line that says the run ended and whether it worked — becomes an icon and a rule instead of the 5.000th grey line. |
...
| **A live cap on parallel runs** | `kanban.max_in_progress` is re-read on every tick, so changing it needs no restart, and `0` finally means *explicitly unbounded* instead of falling back to the memory-derived default. It is set in **Board-Einstellungen**, not on the board. |
| **A model for the whole board** | Pick one model — in **Board-Einstellungen** — and every task run *and* the auto-composer (decompose / specify) use it. A task that pins its own model keeps it. Unset = every profile uses its own model, exactly like the per-task override. The seams it needs are re-attached on every dispatcher tick, so a gateway that could not patch them at boot repairs itself instead of running the wrong model until the next restart. |
...
| **Updates from the page** | Kanban+ compares what is running, what is on disk and what its source repository has, and updates itself with one button. It knows the difference between *there is a new version* and *the new version is already installed and the gateway has not loaded it yet*. **Alle Profile** beside it installs the same version into every Hermes profile on the machine — what `scripts/update.ps1` does, without the shell. |
...
| **A context meter you can read** | The strip carries its total percentage even where the full `used/max` readout does not fit, the hover card ends in the window's own total, and that card opens centred on the pointer rather than pinned to one end of a meter that can span the window. |
...
Beside it, **Alle Profile** does the same thing for every Hermes profile on the machine — the
listing `hermes profile list` prints, each profile installed into its own home (and profiles that
have no copy yet get one). It is the script below, without the shell: the confirmation names every
profile and the version it carries, one profile failing neither stops the others nor hides itself,
and the source is still the one Hermes recorded — never one from the request. A Hermes that does not
expose the home override cannot be pointed at another profile safely; there the button is simply not
offered.

One thing it deliberately does not hide:
...
Beside the switcher, **Board-Einstellungen** (the gear, or *Einstellungen…* in the switcher's menu)
holds the board's scope, the cap on parallel runs and the board model. The page itself only shows
what they are: two settings that decide how every worker on the board runs do not belong between the
header and the lanes, one stray click from changed.
...
per-task model override, the task's skills (editable — core can only set those when the task is
created), the description (editable), result, last summary, dependencies, comments,
...
vertically, with the CLI's own frames redrawn to the width of the panel, inline diffs railed and
coloured, and the worker's exit line marked as the end of the run — and `Esc` brings the details
back. `Esc` again closes the drawer.
...
| `GET /profiles` | Every Hermes profile, its home and the version installed in it (`supported: false` = this Hermes cannot reach another profile from here) |
| `POST /update` `{ref?, all_profiles?}` | Reinstall this plugin from its recorded source (`ref` pins one 40-character SHA). `all_profiles` installs into every profile and answers with one row each |
...
| `PATCH /tasks/{id}` | Move a task to another column, or edit title/body/assignee/priority/model override/**skills** |
...
`skills` is the one field that seam cannot carry: core's `UpdateTaskBody` has no such field, and
nothing in Hermes — CLI, dashboard or agent tool — can change a task's skills after
`kanban_db.create_task` set them. So Kanban+ writes `tasks.skills` itself (`task_skills.py`), the
way core's own per-task override writers write theirs: core's `_normalize_task_skills` decides what
a name may be, an archived task is refused, the row and its event share one transaction, and
`on_kanban_task_updated` fires only after it committed. The same test file asserts that this field
is still one core has no writer for — the day it grows one, this should hand it over.
...
HERMES_AGENT_REPO=/path/to/hermes-agent pytest    # 137 tests against a real checkout
...
kanban-enhancements/      the installable package (this is what lands in $HERMES_HOME/plugins/)
  ...
  profile_update.py       the same install, into every Hermes profile
  task_skills.py          the one task field core's kanban API cannot change
